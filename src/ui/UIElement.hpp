...
namespace re
{
	namespace ui
	{
		...
		@param[in] pos:
			the cursor position (relative to the position of this UIElement). */
		...
		@param[inout] sender:
			the UIElement that caused this event.
		@param[in] enabled:
			true, if the UIElement (or its parent) was enabled.
		@note:
		...
		@note: UIElements do not resize automaticall. */
		class UIElement
		{
			UIElement *_parent;
			...
			/** The border corner vertex data. If there is no border, then it is unused. */
			...
			/** The border width of this UIElemen. */
			float _border_width;
			...
			/** The border image repetition behaviour.
				Specifies the image repitition bahaviour for the sideward axis (horizontal axis for top and bottom border, vertical axis for left and right border). */
			ImageRepetition _border_image_x_repeat_mode;
			/** The border image repetition behaviour.
				Specifies the image repitition bahaviour for the outward axis (vertical axis for top and bottom border, horizontal axis for left and right border). */
			...
			@note:
				...
				Use @[UIElement::enabled] to check whether the UIElement itself is enabled.
			@usage:
				This callback should be used to update the visual appearance of the UIElement. */
		...
		public:
			...
			/** Calls the cursor leave event callbacks and the leave event of all childrens.
				This is called by the onCursorMove function in case the cursor left this UIElements bounds. Call this manually if there is some other kind of reason why the cursor left the element. */
			void onCursorLeave();
			/** Calls the focus changed event callbacks with the new focused state as argument.
			@param[in] focused:
				true, if focus was gained, false, if focus was lost. */
			...
			/** Adds <callback> to the enable / disable event callback list.
			@note:
				The passed value might not represent the actual enabled state of the UIElement.
				Use @[UIElement::enabled] to check whether the UIElement itself is enabled.
			@usage:
			...
			bool hasText() const;
			/** Returns the handle to the Label of this UIElement.
				If this UIElement has no Label, returns null. */
			...
			@return
				the position of this UIElement relative to the origin of its parent. */
			...
			@param[in] background:
				the background image of this UIElement. null for no image (uni color).
			@param[in] blend_color:
				the background color of this UIElement.
			...
			@param[in] border:
				the border texture. Pass null for blank.
			@param[in] blend_color:
				the border (blending) color. also applies to corners.
			@param[in] border_corner:
				the border corner texture. Pass null for blank.
			@param[in] repeat_x:
				the texture repetition @see _border_image_x_repeat_mode.
			@param[in] repeat_y:
				the texture repetition @see _border_image_y_repeat_mode. */
